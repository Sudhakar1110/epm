...
def generate_monthly_scorecards():
    ...
    # Previous month
    if today.month == 1:
        targets.append((12, today.year - 1))
    else:
        targets.append((today.month - 1, today.year))
    
    # Current month
    targets.append((today.month, today.year))
    
    for target_month, target_year in targets:
        # Get all active employees with performance data for this month
        ...
        for employee in employees:
            ...
            if not existing:
                ...
                frappe.logger().info(f"EPMS: Generated scorecard for {employee} ({target_month}/{target_year})")
