...
def portal_setup_common(context):
    ...
    context.is_founder = has_role("EPMS Founder")
...
def portal_annotate_task(row):
    """Add display/class helpers to a Daily Performance dict."""
    row = row or {}
    row["priority_class"] = PRIORITY_CLASS_MAP.get(row.get("priority"), "b-gray")
    row["status_class"] = TASK_STATUS_CLASS_MAP.get(row.get("task_status"), "b-gray")
    row["is_overdue"] = False  # Daily Performance doesn't have overdue concept
    return row
...
def portal_open_tasks(filters=None, limit=None):
    """Daily Performance entries for portal pages."""
    base = {"docstatus": 1}
    if filters:
        base.update(filters)
...
def portal_calendar_tasks(month=None, year=None):
    """Daily Performance entries due in the given month for the calendar view."""
    month = month or getdate(nowdate()).month
    year = year or getdate(nowdate()).year
    ...
    tasks = frappe.get_all(
        ...
        filters={
            "docstatus": 1,
            "date": ["between", [first, last_day]],
        },
        fields=["name", "task_title", "employee_name", "team", "date", "daily_rating", "quality_score", "priority"],
        order_by="date asc",
    )
    by_day = {}
    for t in tasks:
        day = str(t.get("date") or "")[-2:].lstrip("0") or "1"
        by_day.setdefault(day, []).append(t)
    return {"tasks": tasks, "by_day": by_day, "last_day": int(str(last_day)[-2:]) }


def portal_audit_log(limit=60):
    """Recent document changes for EPMS doctypes using Version log.
    ...
    try:
        ...
        frappe.log_error(f"EPMS Audit Log Error: {str(e)}", "EPMS Audit")
        return []
    ...
    for r in rows:
        ...
        try:
            # Only try to parse docchanges if the field exists in the result
            if "docchanges" in r:
                changes = frappe.parse_json(r.get("docchanges") or "[]")
                fields_list = []
                for c in changes:
                    if isinstance(c, dict) and c.get("field"):
                        fields_list.append(c.get("field"))
                if fields_list:
                    changed_fields = ", ".join(fields_list)
        except Exception:
            pass
        ...
        out.append(
            {
                ...
                "by": r.get("modified_by") or r.get("owner") or "",
