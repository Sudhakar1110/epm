...
@frappe.whitelist()
def get_portal_notifications(limit=None):
...
@frappe.whitelist()
def generate_team_scorecards(team=None, month=None, year=None):
    """Generate scorecards for all members of a team for a given month.
    
    Called by TLs/Founders from the scoreboard page to generate on-demand.
    """
    me = frappe.session.user
    user_roles = frappe.get_roles(me)

    if "EPMS Founder" not in user_roles and "EPMS Team Leader" not in user_roles:
        return {"ok": False, "error": _("Only Team Leaders or Founders can generate scorecards.")}
    ...
    if not team:
        return {"ok": False, "error": _("Please select a team.")}
    if month < 1 or month > 12:
        return {"ok": False, "error": _("Month must be between 1 and 12.")}

    # Verify team exists
    team_doc = frappe.db.exists("Team", team)
    if not team_doc:
        return {"ok": False, "error": _("Team not found.")}

    # For Team Leaders, verify they lead this team
    if "EPMS Team Leader" in user_roles and "EPMS Founder" not in user_roles:
        team_leader = frappe.db.get_value("Team", team, "team_leader")
        if team_leader != me:
            return {"ok": False, "error": _("You can only generate scorecards for your own team.")}
    ...
    if not members:
        return {"ok": False, "error": _("No active members found in this team.")}
    ...
    for employee in members:
        ...
        # Check if there's any daily performance data for this employee in this month
        first_day = get_first_day(f"{year}-{month:02d}-01")
        last_day = get_last_day(f"{year}-{month:02d}-01")
        
        has_data = frappe.db.exists(
            "Daily Performance",
            {
                "employee": employee,
                "date": ["between", [first_day, last_day]],
        ...
        try:
            ...
            errors.append(f"{employee}: {str(e)[:100]}")

    frappe.db.commit()

    msg = _("Generated {0} scorecard(s). Skipped {1} (already exist or no data).").format(created, skipped)
    if errors:
        msg += " " + _("Errors: {0}").format("; ".join(errors[:3]))
...
def get_portal_team(team=None):
    """Team detail: info + active members. Accessible to all logged-in users."""
    team = (team or "").strip()
...
@frappe.whitelist()
def submit_team_member_daily_work(member=None, employee_name=None, date=None, task_title=None, actual_hours=None, completion_percentage=None, daily_rating=None, quality_score=None, work_type=None, remarks=None):
    """Submit daily performance entry for a team member (Team Lead or Founder only).
    
    This allows Team Leads to log daily work on behalf of their team members
    at the end of the day.
    """
    me = frappe.session.user
    user_roles = frappe.get_roles(me)
    
    # Only Team Lead or Founder can submit for others
    if "EPMS Founder" not in user_roles and "EPMS Team Leader" not in user_roles:
        return {"ok": False, "error": _("Only Team Lead or Founder can submit work for team members.")}
    ...
    if not member:
        return {"ok": False, "error": _("Member is required.")}
    if not employee_name:
        return {"ok": False, "error": _("Employee name is required.")}
    if not task_title:
        return {"ok": False, "error": _("Please enter the work done.")}
    ...
    try:
        # Get the team of the current user (Team Lead's team)
        team = frappe.db.get_value("Team", {"team_leader": me}, "name")
        if not team:
            # If founder, try to find team from member mapping
            member_team = frappe.db.get_value(
                "Team Member Mapping",
            ...
            if member_team:
                ...
                return {"ok": False, "error": _("Could not determine team.")}

        # Verify the member belongs to this team (for Team Leads)
        if "EPMS Team Leader" in user_roles and "EPMS Founder" not in user_roles:
            is_team_member = frappe.db.exists(
                "Team Member Mapping",
                {"user": member, "team": team, "status": "Active"}
            )
            if not is_team_member:
                return {"ok": False, "error": _("This member is not in your team.")}

        # Check if entry already exists for this member on this date
        if frappe.db.exists(
            "Daily Performance",
            {"employee": member, "date": date, "docstatus": ["!=", 2]},
        ):
            return {"ok": False, "error": _("A daily performance entry already exists for this member on {0}.").format(date)}
        ...
        doc = frappe.get_doc(
            {
                ...
                "priority": "Medium",
                "work_type": (work_type or "").strip() or None,
                "expected_hours": flt(actual_hours or 8),
                "actual_hours": flt(actual_hours or 0),
                "completion_percentage": flt(completion_percentage or 0),
                "daily_rating": flt(daily_rating or 5),
                "quality_score": flt(quality_score or 5),
        ...
        frappe.db.rollback()
        message = str(e).replace("[", "").replace("]", "").strip()
        frappe.log_error(f"EPMS Submit Team Member Work - ValidationError: {message}", "EPMS Submit Team Member Work")
        return {"ok": False, "error": message or _("Could not submit the work. Please check the details.")}
    except Exception as e:
        frappe.db.rollback()
        error_msg = str(e) if e else "Unknown error"
        frappe.log_error(frappe.get_traceback() + f"\nError: {error_msg}", "EPMS Submit Team Member Work")
        return {"ok": False, "error": f"Could not submit the work. Error: {error_msg[:200]}"}
